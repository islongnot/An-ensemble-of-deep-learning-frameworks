...
for i=1:size(cough_data,1)
    y1=cough_data{i};
    melSpectrogram(y1,fs,'Window',hamming(wlen,'periodic')...
                ,'OverlapLength',overlap,'FrequencyRange',[0,16000],NumBands=64)
    
    f=getframe(gca);
    imwrite(f.cdata,['D:\xxxxxx\MelSP\cough\','cough.',num2str(count),'.jpg'],"Quality",100)
    close all
    count=count+1;
end
